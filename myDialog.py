...
class NodeDialog:
    def __init__(self, parent, node_data):
        ...
        ttk.Button(button_frame, text="新增目的地", command=self.add_destination_entry).pack(side=tk.LEFT)
    ...
    def add_destination_entry(self, dest_data=None):
        ...
        # Add enable checkbox
        enable_var = tk.BooleanVar(value=dest_data.get('enable', True) if dest_data else True)
        enable_check = ttk.Checkbutton(entry_frame, text="啟用", variable=enable_var)
        enable_check.pack(side=tk.LEFT, padx=2)
        
        # Add remove button
        remove_btn = ttk.Button(entry_frame, text="移除", 
                              command=lambda f=entry_frame, e=(dest_entry, eng_entry, bureau_entry, enable_var): 
                              self.remove_specific_destination(f, e))
        remove_btn.pack(side=tk.LEFT, padx=2)
        
        self.node_entries.append((dest_entry, eng_entry, bureau_entry, enable_var))
    ...
    def on_ok(self):
        try:
            ...
            for dest_entry, eng_entry, bureau_entry, enable_var in self.node_entries:
            ...
            tk.messagebox.showerror("Error", "座標值必須為數字")
